...
function generateRandomPassword(length) {
  ...
  let letter = characters.indexOf();
  for (let letter of characters) {
    let index = Math.random(letter);
    const indexNew = Math.ceil(index);
    return indexNew;
  }
...
function calculateHypotenuse(a, b) {
  ...
  const gipoten = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
...
function roundObjectValues(obj) {
  ...
  if (typeof obj != "object" || typeof obj == null) {
    ...
    console.Console.log("Помилка: аргумент має бути об'єктом");
    // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    return null;
  }
  ...
  const array = arr.map;
  // ( if (typeof element == 'number'){ Math.round(element)};
  // else { element});
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const newInvest = Math.round(invest);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const reduce = products.reduce(product[price]);
